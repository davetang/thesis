...
The genome is a store of biological information that requires the coordinated activity of enzymes and proteins to bring it to life. Transcription is a highly regulated mechanism that processes the DNA in the genome into RNA (Figure ~\ref{fig:transcription}). The process begins with the transcription preinitiation complex (PIC) forming near the core promoter region, which contains binding sites for the PIC. The core promoter is the closest type of promoter to the transcription start site (TSS); there are also proximal and distal promoters that are further away from the TSS. The PIC contains RNA polymerase II and general transcription factors and positions the RNA polymerase for transcription.

The default state of transcription in eukaryotes is off, meaning that genes are not constantly transribed. Structural properties of DNA make it inaccessible to the PIC; chromatin structure and nucleosome positioning are altered in order for the transcriptional machinery to access parts of the genome for transcription. Transcription factors (TFs) are regulatory proteins that can activate the transcription of DNA by binding to specific DNA sequences (they are also known to negatively regulate transcription but this is less common). For example, the core promoter elements include the TATA box, the initiator element (Inr), the downstream promoter element (DPE), the TFIIB recognition element (BRE) and the CpG island (CGI). Each of these elements are necesarry for transcription; for example the TATA binding protein (TBP) binds to the TATA box and is involved in DNA strand separation during transcription.
