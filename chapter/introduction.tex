...
Since the release of the draft human genome sequence\cite{lander2001initial}, it has been known that only a small fraction of the genome is made up of protein-coding sequences.

This gave rise to 3 paradoxes:
...
If we measure organismal complexity in terms of mental cognition, we observe that complexity is not correlated to the number of chromosomes, the size of genomes, and the number of protein coding genes. Humans have 46 chromosomes and some species of butterflies have over hundreds of chromosomes, such as \textit{Polyommatus atlantica}. In terms of genome size, the species \textit{Amoeba dubia} has one of the largest genomes known with ~670 gb of DNA sequence; humans on the other hand have a genome size of ~3 gb. And lastly humans have roughly the same number of protein-coding genes as \textit{Caenorhabditis elegans}, ~20,000 versus ~21,000, respectively.

While the size of the human genome is ~3 gb, only ~1-2\% of it pertains to protein-coding genes. Roughly 50\% of the human genome is composed of repetitive elements, mainly made up by transposons.
